...
SStruct::SStruct(const std::string &file_content, FileFormat format)
{
    // auto-detect file format and load file
    switch (format)
    {
        case FileFormat_FASTA: LoadFromContentFASTA(file_content); break;
        // case FileFormat_RAW: LoadRAW(filename); break;
        // case FileFormat_BPSEQ: LoadBPSEQ(filename); break;
        default: Error("Not support format yet.");
    }
...
// SStruct::SplitFASTA()
//
// Split FASTA file with multiple sequence into pieces.
//////////////////////////////////////////////////////////////////////
std::vector<std::string> SStruct::SplitFASTA(const std::string &filename){
...
SStruct::FileFormat SStruct::AnalyzeFormat(const std::string &filename)
{
    std::ifstream data(filename.c_str());
...
//////////////////////////////////////////////////////////////////////
// SStruct::LoadFromContentFASTA()
//
// Create object from a FASTA file content.  
//////////////////////////////////////////////////////////////////////

void SStruct::LoadFromContentFASTA(const std::string &file_content)
{
    // clear any previous data
    std::vector<std::string>().swap(names);
    std::vector<std::string>().swap(sequences);
    ...
    // open file for reading
    // std::ifstream data(filename.c_str());
    std::istringstream data(file_content);
    ...
    while (std::getline(data, s))
    {
        ...
        {
            if (sequences.size() == 0) Error("Expected header for FASTA file: %s", file_content.c_str());
            for (size_t i = 0; i < s.length(); i++)
            {
                if (isspace(s[i])) continue;
                sequences.back() += s[i];
