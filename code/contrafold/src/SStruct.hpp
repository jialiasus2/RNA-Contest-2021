...
class SStruct
{
    ...
    void LoadFromContentFASTA(const std::string &file_content);
...
public:
    ...
    SStruct(const std::string &file_content, FileFormat format);
    SStruct(const SStruct &rhs);
    ~SStruct();

    // automatic file format detection
    static FileFormat AnalyzeFormat(const std::string &filename);

    // split FASTA file with multiple sequence into pieces
    static std::vector<std::string> SplitFASTA(const std::string &filename);
