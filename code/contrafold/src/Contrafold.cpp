                          ...
                          std::vector<FileDescription> &descriptions)
{
    ...
    {
        auto format = SStruct::AnalyzeFormat(filenames[i]);
        if(format == SStruct::FileFormat_FASTA){
            //TODO: read multi line
            auto seq = SStruct::SplitFASTA(filenames[i]);
            // printf("Get %lu data items.\n", seq.size());
            for(auto &content:seq){
                descriptions.push_back(FileDescription(filenames[i], content, format, 
    ...
    // printf("MakeFileDescriptions over, description size is %lu\n", descriptions.size());
